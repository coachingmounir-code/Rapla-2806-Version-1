...
function isTeacherInHouseOnDate(teacher, dateStr) {
    ...
    if (teacher.stayStartDate && dateStr < teacher.stayStartDate)
        return false;
    if (teacher.stayEndDate && dateStr > teacher.stayEndDate)
        return false;
    return true;
}
...
const GENERATED_TEACHERS = NEW_TEACHER_NAMES.map((name, index) => {
    ...
    if (name === "Abha")
        id = "teacher-gen-abha-morkoetter";
    else if (name === "Adam")
        id = "teacher-gen-adam-zmuda";
    else if (name === "Alexander")
        id = "teacher-gen-alexander-melior";
    else if (name === "Anjali")
        id = "teacher-gen-anjali-gelzleichter";
    else if (name === "Burnie")
        id = "teacher-gen-burnie-bansemer";
    else if (name === "Hu")
        id = "teacher-gen-hu-buerkle";
    else if (name === "Karuna")
        id = "teacher-gen-karuna-wapke";
    else if (name === "Mounir")
        id = "teacher-gen-mouniir-jaber";
    else if (name === "Narayani")
        id = "teacher-gen-narayani-kedenburg";
    else if (name === "Nirmaya")
        id = "teacher-gen-nirmaya-fodor";
    else if (name === "Pranava")
        id = "teacher-gen-pranava-pauly";
    else if (name === "Teresa")
        id = "teacher-gen-teresa-allgaeu";
    else if (name === "Ulrich")
        id = "teacher-gen-ulrich-nebel";
    ...
    const isYogaTeacher = isSevaka ? !(name.toLowerCase().includes('teresa') ||
        name.toLowerCase().includes('hu') ||
        name.toLowerCase().includes('mounir') ||
        name.toLowerCase().includes('adam') ||
        name.toLowerCase().includes('satyam')) : true;
...
const generateDefaultCourses = () => {
    const getTeacherIdByName = (shortName) => {
        ...
        if (nameLower === 'mouniir' || nameLower === 'mounir')
    ...
    getOrCreateUpcomingWeekPlans: () => {
        ...
        for (let w = 0; w < 4; w++) {
            ...
            if (!plan) {
                ...
                const getLocalDateForPlanDay = (dOfWeek) => {
                    let dayOff = 0;
                    if (dOfWeek === 5)
                        dayOff = 0;
                    else if (dOfWeek === 6)
                ...
                plan = {
                    ...
                    courses: template.courses
                        ...
                        if (isPranayama) {
                            ...
                            if ((0, ylaData_1.isDateInYlaRange)(cDate))
                                return false;
                        }
                        return true;
                    })
                        .map(c => {
                        const cDate = getLocalDateForPlanDay(c.dayOfWeek);
                        const isAbhaInYla = (c.teacherId === 'teacher-gen-abha-morkoetter' || c.teacherId === 'abha') && (0, ylaData_1.isDateInYlaRange)(cDate);
                        const inYla = (0, ylaData_1.isDateInYlaRange)(cDate);
                        const nameLower = c.name.toLowerCase();
                        let roomId = c.roomId;
                        if (inYla) {
