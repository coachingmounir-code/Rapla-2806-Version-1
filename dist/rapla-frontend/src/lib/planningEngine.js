...
function validateAssignment(teacher, course, allCourses, seminarLeaderIds = [], targetWeekCode, teachers) {
    ...
    if (teacher.name.toLowerCase().includes('harishakti')) {
        ...
        if (day === 1) { // Montag
            ...
            const otherMondayAssignments = allCourses.filter(c => c.teacherId === teacher.id && c.id !== course.id && c.dayOfWeek === 1);
            ...
            if (courseStart < timeToMinutes('12:00')) {
                const melanieActiveOnFriday = allCourses.some(c => {
                    if (c.dayOfWeek !== 5 || !c.teacherId)
                        return false;
                    const t = db_1.db.getTeachers().find(x => x.id === c.teacherId);
    ...
    if (teacher.name.toLowerCase().includes('karuna')) {
        ...
        if (isYogaClass) {
            const otherYogaClassesOnDay = allCourses.filter(c => c.teacherId === teacher.id && c.id !== course.id && c.dayOfWeek === day && c.style.toLowerCase() !== 'meditation');
            ...
            const weeklyYogaClasses = allCourses.filter(c => c.teacherId === teacher.id && c.id !== course.id && c.style.toLowerCase() !== 'meditation');
    ...
    if (isSatsangEinfuehrung) {
        if (course.dayOfWeek === 5) {
            ...
            const pranava = (teachers || db_1.db.getTeachers()).find(t => t.name.toLowerCase().includes('pranava'));
            if (typeof window !== 'undefined' && targetWeekCode && pranava) {
                try {
                    ...
                    if (saved) {
                        const sevafreiList = JSON.parse(saved);
                        const activeAbsence = sevafreiList.find((entry) => entry.teacherId === pranava.id &&
    ...
    if (course.name === 'Satsang' && course.startTime === '20:00' && [3, 4, 5, 6, 0].includes(course.dayOfWeek)) {
        ...
        const karuna = (teachers || db_1.db.getTeachers()).find(t => t.name.toLowerCase().includes('karuna'));
        if (typeof window !== 'undefined' && targetWeekCode && karuna) {
            try {
                ...
                if (saved) {
                    const sevafreiList = JSON.parse(saved);
                    const activeAbsence = sevafreiList.find((entry) => entry.teacherId === karuna.id &&
                        courseDate >= entry.startDate &&
                        courseDate <= entry.endDate);
                    if (activeAbsence) {
                        isKarunaAbsent = true;
    ...
    if (isMeditationCourse) {
        ...
        const isQualified = teacher.specialties.some(spec => spec.toLowerCase() === course.style.toLowerCase());
        if (!isQualified) {
            conflicts.push({
                type: 'hard',
    ...
    const roomConflicts = validateRoomRules(course, allCourses, teachers || db_1.db.getTeachers());
